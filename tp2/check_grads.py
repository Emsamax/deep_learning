...
for exp in lambdas:
    lambda_val = 10**exp
    print(f"\nL2Regularizer e{exp} (lambda = {lambda_val:.3e})")
    l2reg = layers.L2Regularizer(x, lambda_val, 'L2reg')
    print("Check grad wrt params")
...
- TOO LARGE (e.g., 1e-1): Weights → 0, simple model → UNDERFITTING (high bias)
- TOO SMALL (e.g., 1e-3): High weights, complex model → OVERFITTING (high variance)
- OPTIMAL: Best balance → highest test accuracy and generalizable filters
"""